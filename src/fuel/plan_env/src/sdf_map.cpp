...
namespace fast_planner
{
  ...
  void SDFMap::initMap(std::shared_ptr<rclcpp::Node> nh)
  {
    ...
    // nh->declare_parameter("sdf_map/resolution", -1.0);
    // nh->declare_parameter("sdf_map/map_size_x", -1.0);
    // nh->declare_parameter("sdf_map/map_size_y", -1.0);
    // nh->declare_parameter("sdf_map/map_size_z", -1.0);
    // nh->declare_parameter("sdf_map/obstacles_inflation", -1.0);
    // nh->declare_parameter("sdf_map/local_bound_inflate", 1.0);
    // nh->declare_parameter("sdf_map/local_map_margin", 1);
    // nh->declare_parameter("sdf_map/ground_height", 1.0);
    // nh->declare_parameter("sdf_map/default_dist", 5.0);
    ...
    // nh->declare_parameter("sdf_map/p_hit", 0.70);
    // nh->declare_parameter("sdf_map/p_miss", 0.35);
    // nh->declare_parameter("sdf_map/p_min", 0.12);
    // nh->declare_parameter("sdf_map/p_max", 0.97);
    // nh->declare_parameter("sdf_map/p_occ", 0.80);
    ...
    for (int i = 0; i < 3; ++i)
    {
      // nh->declare_parameter("sdf_map/box_min_" + axis[i], mp_->map_min_boundary_[i]);
      // nh->declare_parameter("sdf_map/box_max_" + axis[i], mp_->map_max_boundary_[i]);
      nh->get_parameter("sdf_map/box_min_" + axis[i], mp_->box_mind_[i]);
      nh->get_parameter("sdf_map/box_max_" + axis[i], mp_->box_maxd_[i]);
    }
    posToIndex(mp_->box_mind_, mp_->box_min_);
    posToIndex(mp_->box_maxd_, mp_->box_max_);
    // Initialize ROS2 wrapper
    mr_->setMap(this);
    mr_->node_ = nh;
    mr_->init();
