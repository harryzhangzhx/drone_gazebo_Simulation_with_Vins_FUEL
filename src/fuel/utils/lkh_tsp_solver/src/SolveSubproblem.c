                    ...
                    GainType *GlobalBestCost)
{
    ...
    int NewDimension = 0, OldDimension = 0, Number, i, InitialTourEdges = 0, AscentCandidatesSaved = AscentCandidates, InitialPeriodSaved = InitialPeriod, MaxTrialsSaved = MaxTrials;
        ...
        if (Cost < Optimum || (Cost != Optimum && OutputTourFileName))
        {
            ...
            if (TraceLevel >= 1)
            {
                ...
                if (OptimumSaved != MINUS_INFINITY && OptimumSaved != 0)
                    printff(", Gap = %0.4f%%",
                            100.0 * (GlobalCost - OptimumSaved) / OptimumSaved);
