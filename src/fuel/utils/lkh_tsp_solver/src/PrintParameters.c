...
void PrintParameters()
{
    ...
    printff("CANDIDATE_SET_TYPE = %s%s\n",
            CandidateSetType == ALPHA ? "ALPHA" : CandidateSetType == DELAUNAY ? "DELAUNAY"
    ...
    printff("INITIAL_TOUR_ALGORITHM = %s\n",
            InitialTourAlgorithm == BORUVKA ? "BORUVKA" : InitialTourAlgorithm == GREEDY         ? "GREEDY"
    ...
    printff("RECOMBINATION = %s\n", Recombination == IPT ? "IPT" : Recombination == GPX2 ? "GPX2"
                                                                                         : "UNKNOWN");
    ...
    if (SubproblemSize == 0)
        ...
        printff("SUBPROBLEM_SIZE = %d%s%s%s\n", SubproblemSize,
                DelaunayPartitioning ? " DELAUNAY" : KarpPartitioning     ? " KARP"
