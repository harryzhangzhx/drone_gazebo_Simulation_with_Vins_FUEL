...
void BsplineOptimizer::setParam(const std::shared_ptr<rclcpp::Node>& nh) {
  ...
  nh->get_parameter_or("optimization/ld_smooth", ld_smooth_, -1.0);
  nh->get_parameter_or("optimization/ld_dist",   ld_dist_,   -1.0);
  nh->get_parameter_or("optimization/ld_feasi",  ld_feasi_,  -1.0);
  nh->get_parameter_or("optimization/ld_start",  ld_start_,  -1.0);
  nh->get_parameter_or("optimization/ld_end",    ld_end_,    -1.0);
  nh->get_parameter_or("optimization/ld_guide",  ld_guide_,  -1.0);
  nh->get_parameter_or("optimization/ld_waypt",  ld_waypt_,  -1.0);
  nh->get_parameter_or("optimization/ld_view",   ld_view_,   -1.0);
  nh->get_parameter_or("optimization/ld_time",   ld_time_,   -1.0);

  nh->get_parameter_or("optimization/dist0",   dist0_,   -1.0);
  nh->get_parameter_or("optimization/max_vel", max_vel_, -1.0);
  nh->get_parameter_or("optimization/max_acc", max_acc_, -1.0);
  nh->get_parameter_or("optimization/dlmin",   dlmin_,   -1.0);
  nh->get_parameter_or("optimization/wnl",     wnl_,     -1.0);

  nh->get_parameter_or("optimization/max_iteration_num1",  max_iteration_num_[0],  -1);
  nh->get_parameter_or("optimization/max_iteration_num2",  max_iteration_num_[1],  -1);
  nh->get_parameter_or("optimization/max_iteration_num3",  max_iteration_num_[2],  -1);
  nh->get_parameter_or("optimization/max_iteration_num4",  max_iteration_num_[3],  -1);
  nh->get_parameter_or("optimization/max_iteration_time1", max_iteration_time_[0], -1.0);
  nh->get_parameter_or("optimization/max_iteration_time2", max_iteration_time_[1], -1.0);
  nh->get_parameter_or("optimization/max_iteration_time3", max_iteration_time_[2], -1.0);
  nh->get_parameter_or("optimization/max_iteration_time4", max_iteration_time_[3], -1.0);

  nh->get_parameter_or("optimization/algorithm1", algorithm1_, -1);
  nh->get_parameter_or("optimization/algorithm2", algorithm2_, -1);
...
void BsplineOptimizer::optimize(Eigen::MatrixXd& points, double& dt, const int& cost_function,
  ...
  if (start_state_.empty()) {
    RCLCPP_ERROR(nh_->get_logger(), "Initial state undefined!");
    return;
  }
  ...
  if (variable_num_ <= 0) {
    RCLCPP_ERROR(nh_->get_logger(), "Empty varibale to optimization solver.");
    return;
  }
...
void BsplineOptimizer::optimize() {
  ...
  if (cost_function_ & MINTIME) {
    std::cout << "Iter num: " << iter_num_ << ", time: " << (nh_->now() - t1).seconds()
  ...
  //   RCLCPP_INFO_STREAM("iter num: " << iter_num_);
}
