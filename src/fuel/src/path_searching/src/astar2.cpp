...
void Astar::init(const std::shared_ptr<rclcpp::Node>& nh, const EDTEnvironment::Ptr& env) {
  ...
  cout << "Path_searching Astar2 init completed" << endl;
}
...
int Astar::search(const Eigen::Vector3d& start_pt, const Eigen::Vector3d& end_pt) {
  ...
  cout << "FUEL Astar called. Start: " << start_pt.transpose() << "\nEnd: " << end_pt.transpose() << endl;
  ...
  while (!open_set_.empty()) {
    ...
    if (reach_end) {
      ...
      std::cout << "FUEL Astar target reached" << std::endl;
      return REACH_END;
    }
