...
FrontierFinder::FrontierFinder(const shared_ptr<EDTEnvironment>& edt, const std::shared_ptr<rclcpp::Node>& nh) {
  nh_ = nh;
  this->edt_env_ = edt;
  ...
  nh->get_parameter("frontier/cluster_min", cluster_min_);

  nh->get_parameter("frontier/cluster_size_xy", cluster_size_xy_);

  nh->get_parameter("frontier/cluster_size_z", cluster_size_z_);

  nh->get_parameter("frontier/min_candidate_dist", min_candidate_dist_);

  nh->get_parameter("frontier/min_candidate_clearance", min_candidate_clearance_);

  nh->get_parameter("frontier/candidate_dphi", candidate_dphi_);

  nh->get_parameter("frontier/candidate_rmax", candidate_rmax_);

  nh->get_parameter("frontier/candidate_rmin", candidate_rmin_);

  nh->get_parameter("frontier/candidate_rnum", candidate_rnum_);

  nh->get_parameter("frontier/down_sample", down_sample_);

  nh->get_parameter("frontier/min_visib_num", min_visib_num_);

  nh->get_parameter("frontier/min_view_finish_fraction", min_view_finish_fraction_);
...
void FrontierFinder::searchFrontiers() {
  ...
  RCLCPP_WARN_THROTTLE(nh_->get_logger(), *nh_->get_clock(), 5000, "Frontier t: %lf", (nh_->now() - t1).seconds());
}
