...
namespace fast_planner
{
  ...
  void Graph::dijkstraSearch(const int &start, const int &goal, vector<YawVertex::Ptr> &path)
  {
    ...
    std::cout << "Dijkstra can't find path!" << std::endl;
    std::cout << "Exiting due to failure." << std::endl;
    // RCLCPP_ERROR(nh_->get_logger(), "Dijkstra can't find path!");
    // RCLCPP_FATAL(nh_->get_logger(), "Exiting due to failure.");
  }

  HeadingPlanner::HeadingPlanner(const std::shared_ptr<rclcpp::Node> &nh)
  {
    nh_ = nh;

    frontier_pub_ = nh->create_publisher<sensor_msgs::msg::PointCloud2>("/heading_planner/frontier", 20);
    visib_pub_ = nh->create_publisher<sensor_msgs::msg::PointCloud2>("/heading_planner/visib", 20);
    box_pub_ = nh->create_publisher<visualization_msgs::msg::Marker>("/heading_planner/box", 20);

    nh->get_parameter("heading_planner/yaw_diff", yaw_diff_);

    nh->get_parameter("heading_planner/lambda1", lambda1_);

    nh->get_parameter("heading_planner/lambda2", lambda2_);

    nh->get_parameter("heading_planner/half_vert_num", half_vert_num_);

    nh->get_parameter("heading_planner/max_yaw_rate", max_yaw_rate_);

    nh->get_parameter("heading_planner/w", w_);

    nh->get_parameter("heading_planner/weight_type", weight_type_);
                                       ...
                                       vector<double> &path)
  {
    ...
    for (int i = 0; i < yaws.size(); ++i)
    {
      ...
      { // inter vertice
        ...
        { // evaluate info gain in parallel
    ...
    for (int x = lbi[0]; x <= ubi[0]; ++x)
      for (int y = lbi[1]; y <= ubi[1]; ++y)
        for (int z = lbi[2]; z <= ubi[2]; ++z)
        {
          ...
          if (flag == 1)
          { // visited cell, fetch visibility directly
