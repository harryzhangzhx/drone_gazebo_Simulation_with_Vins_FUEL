...
static int compareFromTo(const void *e1, const void *e2)
{
    ...
    return From1 < From2 ? -1 : From1 > From2 ? 1
                                              : To1 - To2;
}
    ...
    for (i = 0; i < Dimension; i++)
    {
        ...
        do
        {
            ...
            if (u->id < v->id)
            {
                ...
                EdgeSet[j++].Cost = FixedOrCommon(From, To) ? INT_MIN : Distance(From, To) * Precision + From->Pi + To->Pi;
            }
        } while ((e = Next(e, u)) != e_start);
    }
    free_memory();
        ...
        if (N != FirstNode)
        {
            ...
            do
            {
                ...
                if (WeightType == GEO || WeightType == GEO_MEEUS)
                    From->Y =
                        (int)From->Y + 5.0 * (From->Y - (int)From->Y) / 3.0;
                From->Y += From->Y > 0 ? -180 : 180;
                if (WeightType == GEO || WeightType == GEO_MEEUS)
                    From->Y =
                        (int)From->Y + 3.0 * (From->Y - (int)From->Y) / 5.0;
            ...
            for (i = 0; i < Dimension; i++)
            {
                ...
                do
                {
                    ...
                    if (!bsearch(&Key, EdgeSet, Count1, sizeof(Edge),
                                 compareFromTo))
                    {
