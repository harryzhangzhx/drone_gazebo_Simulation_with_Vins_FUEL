...
void SolveRoheSubproblems()
{
    ...
    if (WeightType == GEO || WeightType == GEOM || WeightType == GEO_MEEUS || WeightType == GEOM_MEEUS)
    {
        N = FirstNode;
        do
        {
                           ...
                           int Subproblem, int start, int end)
{
    ...
    else
    {
        ...
        if (N->Axis == 0 ? N->X >= XMin : N->Axis == 1 ? N->Y >= YMin
                                                       : N->Z >= ZMin)
            MakeSubproblem(XMin, XMax, YMin, YMax, ZMin, ZMax, Subproblem,
                           start, mid - 1);
        if (N->Axis == 0 ? N->X <= XMax : N->Axis == 1 ? N->Y <= YMax
                                                       : N->Z <= ZMax)
            MakeSubproblem(XMin, XMax, YMin, YMax, ZMin, ZMax, Subproblem,
                           mid + 1, end);
    }
