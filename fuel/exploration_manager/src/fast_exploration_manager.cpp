...
namespace fast_planner
{
  ...
  void FastExplorationManager::initialize(const std::shared_ptr<rclcpp::Node> &node)
  {
    planner_manager_.reset(new FastPlannerManager);
    planner_manager_->initPlanModules(node);
    edt_environment_ = planner_manager_->edt_environment_;
    sdf_map_ = edt_environment_->sdf_map_;
    frontier_finder_.reset(new FrontierFinder(edt_environment_, node));
    ...
    // this->declare_parameter<type>("parameter_name", default_value);
    // node->declare_parameter<bool>("exploration.refine_local", true);
    // node->declare_parameter<int>("exploration.refined_num", -1);
    // node->declare_parameter<double>("exploration.refined_radius", -1.0);
    // node->declare_parameter<int>("exploration.top_view_num", -1);
    // node->declare_parameter<double>("exploration.max_decay", -1.0);
    // node->declare_parameter<std::string>("exploration.tsp_dir", "null");
    // node->declare_parameter<double>("exploration.relax_time", 1.0);
    // node->declare_parameter<double>("exploration.vm", -1.0);
    // node->declare_parameter<double>("exploration.am", -1.0);
    // node->declare_parameter<double>("exploration.yd", -1.0);
    // node->declare_parameter<double>("exploration.ydd", -1.0);
    // node->declare_parameter<double>("exploration.w_dir", -1.0);
    // node->declare_parameter<bool>("cmu_exploration", false);

    // this->get_parameter("parameter_name", variable);
    node->get_parameter("exploration/refine_local", ep_->refine_local_);
    node->get_parameter("exploration/refined_num", ep_->refined_num_);
    node->get_parameter("exploration/refined_radius", ep_->refined_radius_);
    node->get_parameter("exploration/top_view_num", ep_->top_view_num_);
    node->get_parameter("exploration/max_decay", ep_->max_decay_);
    node->get_parameter("exploration/tsp_dir", ep_->tsp_dir_);
    node->get_parameter("exploration/relax_time", ep_->relax_time_);
    node->get_parameter("exploration/vm", ViewNode::vm_);
    node->get_parameter("exploration/am", ViewNode::am_);
    node->get_parameter("exploration/yd", ViewNode::yd_);
    node->get_parameter("exploration/ydd", ViewNode::ydd_);
    node->get_parameter("exploration/w_dir", ViewNode::w_dir_);
    node->get_parameter("cmu_exploration", cmu_exploration);

    // endpose_pub_ = nh.advertise<geometry_msgs::Point>("/chen/plan_pose", 10);
    endpose_pub_ = node->create_publisher<geometry_msgs::msg::Point>("/chen/plan_pose", 10);

    // cmu_pose_pub_ = nh.advertise<geometry_msgs::PointStamped>("/way_point", 10);
    cmu_pose_pub_ = node->create_publisher<geometry_msgs::msg::PointStamped>("/way_point", 10);
    ViewNode::astar_.reset(new Astar);
    ViewNode::astar_->init(node, edt_environment_);
      ...
      const Vector3d &pos, const Vector3d &vel, const Vector3d &acc, const Vector3d &yaw)
  {
    ...
    else if (ed_->points_.size() == 1)
    {
      ...
      RCLCPP_ERROR(rclcpp::get_logger("FastExplorationManager"), "Empty destination.");
    ...
    {
      RCLCPP_ERROR(rclcpp::get_logger("FastExplorationManager"), "No path to next viewpoint");
      return FAIL;
    }
  ...
  void FastExplorationManager::shortenPath(vector<Vector3d> &path)
  {
    if (path.empty())
    {
      RCLCPP_ERROR(rclcpp::get_logger("FastExplorationManager"), "Empty path to shorten");
      return;
    }
